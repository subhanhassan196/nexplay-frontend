...
const sizeClasses: Record<MessengerSize, string> = {
  small: "w-[360px] h-[520px]",
  medium: "w-[400px] h-[600px]",
  large: "w-[440px] h-[680px]",
};
...
export function MessengerPanel() {
  ...
  async function handleSend(content: string, attachmentUrls?: string[]) {
    await sendMessage(content, replyTo?.id, attachmentUrls);
    setReplyTo(null);
  }
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className={cn(
            "fixed bottom-24 right-4 z-[60] flex flex-col overflow-hidden rounded-3xl border border-white/10 bg-background/95 shadow-2xl backdrop-blur-xl sm:right-6",
