...
import { type AssignedAgentDTO, type UploadedAttachment, messengerApi, type MessageDTO, type BootstrapDTO, type ConversationState } from "@/lib/api/messenger";
...
interface MessengerContextValue {
  ...
  /** The agent handling this thread, once one is assigned. */
  agent: AssignedAgentDTO | null;
  ...
  /** Uploads any chat file (image, document, voice) and returns metadata. */
  uploadFile: (file: File, durationSeconds?: number) => Promise<UploadedAttachment>;
...
export function MessengerProvider({ children }: { children: ReactNode }) {
  ...
  const [agent, setAgent] = useState<AssignedAgentDTO | null>(null);
  ...
  useEffect(() => {
    ...
    // Support availability is broadcast when an admin flips the toggle,
    // so the status dot updates live rather than on next page load.
    const handleSettings = (payload: { isOnline: boolean; supportHours?: string; offlineMessage?: string }) => {
      setBootstrap((prev) =>
        prev
          ? {
              ...prev,
              isOnline: payload.isOnline,
              supportHours: payload.supportHours ?? prev.supportHours,
              offlineMessage: payload.offlineMessage ?? prev.offlineMessage,
            }
          : prev
      );
    };

    socket.on(SOCKET_EVENTS.SUPPORT_SETTINGS, handleSettings);
    socket.on(SOCKET_EVENTS.MESSAGE_NEW, handleNewMessage);
    socket.on(SOCKET_EVENTS.MESSAGE_UPDATED, handleUpdatedMessage);
    socket.on(SOCKET_EVENTS.TYPING, handleTyping);

    return () => {
      socket.off(SOCKET_EVENTS.SUPPORT_SETTINGS, handleSettings);
  ...
  const sendMessage = useCallback(
    async (content: string, replyToId?: string, attachmentUrls?: string[], attachments?: UploadedAttachment[]) => {
      setIsSending(true);
      try {
        const { data } = await messengerApi.sendMessage(
          content,
          replyToId,
          attachmentUrls,
  ...
  const uploadFile = useCallback(async (file: File, durationSeconds?: number) => {
    const { data } = await messengerApi.uploadFile(file, durationSeconds);
