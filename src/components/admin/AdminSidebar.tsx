...
import { MessagesSquare, Megaphone, Link2, Settings, LayoutDashboard, ArrowLeft, Activity, BarChart3, FileText, Image, Globe, HeartPulse, Gamepad2, Gauge, Users, Trophy, Gift, MessagesSquare as CommunityIcon } from "lucide-react";
...
interface AdminSidebarProps {
  /// Controls the mobile drawer. On desktop the sidebar is always visible
...
export function AdminSidebar({ mobileOpen, onClose }: AdminSidebarProps = {}) {
  ...
  return (
    <aside
      className={cn(
        "flex h-screen w-60 flex-col border-r border-white/10 bg-surface/40 py-4 backdrop-blur-xl transition-transform duration-300",
        // Phones get a slide-over: a permanent rail eats too much of a
        // narrow screen, and the inbox needs every pixel.
        "fixed inset-y-0 left-0 z-[85] md:static md:w-16 md:translate-x-0 lg:w-60",
    ...
    >
      ...
      <nav className="flex flex-1 flex-col gap-4 overflow-y-auto px-2 lg:px-3">
        {navGroups.map((group) => (
          <div key={group.label} className="flex flex-col gap-1">
            <p className="block px-3 pt-1 text-[10px] font-semibold uppercase tracking-wider text-muted/60 md:hidden lg:block">
              {group.label}
            </p>
            {group.items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="relative block md:hidden lg:block">{item.label}</span>
                </Link>
              );
            })}
          </div>
      ...
      <div className="px-2 lg:px-3">
        ...
        >
          ...
          <span className="block md:hidden lg:block">Back to App</span>
        </Link>
      </div>
    </aside>
  );
