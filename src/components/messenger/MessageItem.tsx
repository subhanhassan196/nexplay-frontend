    ...
    <div className={cn("group relative flex gap-2 px-3 py-1", isMine && "flex-row-reverse")}>
      ...
      <div className={cn("flex max-w-[78%] flex-col gap-1", isMine && "items-end")}>
        ...
        <div className="relative">
          <div
            // `whitespace-pre-wrap` keeps the sender's line breaks;
            // `overflowWrap: anywhere` breaks a long unbroken string (a
            // pasted URL or key) instead of letting it stretch the bubble
            // past the panel edge.
          ...
          >
            {/* Structured attachments — image, document or voice note */}
            {!isDeleted && message.attachments?.length > 0 && (
              <MessageAttachments attachments={message.attachments} isMine={isMine} onImageClick={setLightbox} />
            )}

            {/* Legacy URL-only attachments from before typed metadata existed */}
            {!isDeleted && message.attachments?.length === 0 && message.attachmentUrls.length > 0 && (
