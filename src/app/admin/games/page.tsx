...
export default function AdminGamesPage() {
  ...
  async function handleDeletePermanently() {
    ...
    try {
      await adminCatalogApi.deleteGamePermanently(confirmDelete.id);
      setConfirmDelete(null);
      await load();
      toast({ variant: "success", title: "Game deleted" });
    } catch (err) {
      // The server refuses when a tournament still references the game,
      // and says so — surface that message rather than a generic failure.
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-xl font-bold text-white sm:text-2xl">Game Catalog</h1>
        ...
        <div className="mb-6 rounded-2xl border border-primary/30 bg-surface/60 p-5">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <ImageUploadField
              ...
              onChange={(v) => setEditing((s) => ({ ...s!, coverImageUrl: v }))}
              folder="games"
              hint="Shown on game cards — landscape works best"
            />
            <ImageUploadField
              label="Logo"
              value={editing.logoUrl ?? ""}
              onChange={(v) => setEditing((s) => ({ ...s!, logoUrl: v }))}
              folder="games"
              hint="Square icon, optional"
            />
            <ImageUploadField
              label="Banner"
              value={editing.bannerUrl ?? ""}
              onChange={(v) => setEditing((s) => ({ ...s!, bannerUrl: v }))}
              folder="games"
              hint="Wide hero art for the game page, optional"
            />
        ...
        <div className="space-y-2">
          {games.map((g) => (
            ...
            >
              ...
              <button
                // Fall back to the nested category's id: older API responses
                // returned the relation without the flat `categoryId`, which
                // made the dropdown read "Uncategorised" and then wiped the
                // game's category on save.
                onClick={() => setEditing({ ...g, categoryId: g.categoryId ?? g.category?.id ?? null })}
              ...
              <div className="flex shrink-0 items-center gap-1">
                ...
                {g.status === "ARCHIVED" ? (
                  ...
                  <>
                  <button
                    onClick={() => setConfirmDelete({ id: g.id, title: g.title })}
                    className="rounded-lg p-2 text-muted hover:text-danger"
                    aria-label={`Delete ${g.title} permanently`}
                    title="Delete permanently"
                  >
                    <Trash2 className="h-4 w-4" />
                  </button>
                  <button onClick={() => handleArchive(g.id)} className="rounded-lg p-2 text-muted hover:text-accent" aria-label="Archive" title="Archive (recoverable)">
      ...
      {/* Permanent deletion can't be undone, so it gets an explicit
          confirmation naming the game and pointing at archiving. */}
      {confirmDelete && (
        <div
        ...
        >
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-2xl border border-white/10 bg-background p-6">
            <div className="mb-3 flex items-center gap-3">
              ...
              <h2 className="font-display text-lg font-semibold text-white">Delete this game?</h2>
            </div>
            <p className="mb-2 text-sm text-white">{confirmDelete.title}</p>
            <p className="mb-5 text-sm text-muted">
              This removes the game and its artwork from the catalog permanently. If you only want to hide it from
              players, archive it instead — archived games can be restored later.
            </p>
            <div className="flex flex-wrap justify-end gap-2">
              ...
              <button
                onClick={() => {
                  handleArchive(confirmDelete.id);
                  setConfirmDelete(null);
                }}
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-lg bg-danger px-4 py-2 text-sm font-medium text-white hover:opacity-90"
              >
